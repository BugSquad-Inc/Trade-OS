...
| TOS-REP-API-019 | audit_repo.py | Append-only centralized compliance and security audit trail repository |
...
| TOS-REP-API-009 | journey_repo | Stage event persistence and immutable history querying |
...
| TOS-RTE-API-026 | audit.py | FastAPI routes for immutable audit events retrieval and compliance stats |
...
| TOS-RTE-API-014 | journey.router | FastAPI routes for state inspection, stage transitions, and immutable audit history |
...
| TOS-SCH-API-015 | Journey Schemas | Pydantic v2 schemas for journey actions, blockers, states, transitions, and history |
| TOS-SCH-API-016 | Product Matrix Schemas | Pydantic v2 schemas for physical specs, chemical safety, traceability, and DPP public tokens |
...
| TOS-SCH-API-021 | AuditEvent & AuditStats Schemas | Pydantic v2 schemas for immutable audit events, logging, and audit trail metrics |
...
| TOS-TST-API-009 | test_journey_engine.py | Pytest suite for golden paths, invalid transition rejection, idempotency, and history |
| TOS-TST-API-010 | test_product_matrix_rbac.py | Pytest suite for product matrix creation, public DPP token lookup, and RBAC 7-role permissions |
| TOS-TST-API-011 | test_provenance_analyst_loop.py | Pytest suite for data freshness calculation, 90-day SLA, and analyst review workflow |
| TOS-TST-API-012 | test_scoring_v2_explainability.py | Pytest suite for non-compensatory compliance gates, counter-factual recommendations, and explainability weights |
| TOS-TST-API-013 | test_outreach_v2_compliance_pack.py | Pytest suite for outreach modes, DIN 5008 German, and compliance pack bundle generation |
| TOS-TST-API-014 | test_s7_production_readiness.py | Pytest suite for full lifecycle E2E integration, security headers, and compression |

---
*Auto-generated by update.py — do not edit manually.*
*Last updated: 2026-09-01T15:40:49Z*